  ...
  # end
    ...
    context '新規登録できない場合' do
      ...
      it 'お名前カナ(全角)名字のフリガナが空の場合、エラーになること' do
        @user.last_name_kana = ''
        @user.valid?
        expect(@user.errors.full_messages).to include("Last name kana can't be blank", "Last name kana can't be blank", "Last name kana は全角カタカナで入力して下さい。")
      end

      it 'お名前カナ(全角)名前のフリガナが空の場合、エラーになること' do
        @user.first_name_kana = ''
        @user.valid?
        expect(@user.errors.full_messages).to include("First name kana can't be blank", "First name kana can't be blank", "First name kana は全角カタカナで入力して下さい。")
      end

      it 'お名前(全角)が全角（漢字・ひらがな・カタカナ）で入力されていないとエラーになること' do
